...
import { User, Shirt, Ruler, BarChart3, Video, FileText, Calendar, Globe } from "lucide-react";
import { POSITIONS, COUNTRIES, PREFERRED_FOOT, FOOTBALL_LEVELS } from "@/lib/constants";
import type { StepProps } from "@/lib/types";

function getLabel(list: { value: string; label: string }[], code: string): string {
  ...
  return (
    <div className="rounded-xl border p-5" style={{ backgroundColor: "var(--bg-card)", borderColor: "var(--border)" }}>
      <div className="flex items-center gap-3 mb-3">
        ...
        <h3 className="text-sm font-semibold" style={{ color: "var(--fg-heading)" }}>{title}</h3>
      </div>
...
function DataRow({ label, value }: { label: string; value: string }) {
  ...
  return (
    <div className="flex flex-col py-1.5">
      ...
      <span className="text-sm" style={{ color: isEmpty ? "var(--fg-muted)" : "var(--fg-heading)", fontStyle: isEmpty ? "italic" : "normal" }}>
        {value || "Not provided"}
      </span>
    </div>
  );
...
export default function Step12ReviewSubmit({ data }: StepProps) {
  const sections = [
    ...
    },
    {
      ...
      fields: [
        ...
        { label: "Nationality", value: getLabel(COUNTRIES, data.nationality) },
        { label: "Country of Residence", value: getLabel(COUNTRIES, data.country) },
        { label: "City", value: data.city || "" },
        { label: "Phone", value: data.phoneNumber || "" },
      ],
    },
    {
      title: "Football Profile", icon: Shirt,
      fields: [
        { label: "Primary Position", value: getLabel(POSITIONS, data.currentPosition) },
        { label: "Preferred Foot", value: getLabel(PREFERRED_FOOT, data.preferredFoot) },
        { label: "Current Level", value: getLabel(FOOTBALL_LEVELS, data.currentLevel) },
    ...
    {
      title: "Physical Attributes", icon: Ruler,
      fields: [
    ...
    {
      title: "Playing Style", icon: Globe,
      fields: [
        ...
        { label: "Favorite Position", value: getLabel(POSITIONS, data.favoritePosition) },
    ...
    },
    {
      ...
      fields: [
        ...
        ...(data.videos || []).slice(0, 3).map((url: string, i: number) => ({ label: `Video ${i + 1}`, value: url })),
      ],
    },
    {
    ...
    },
    {
      ...
      fields: [
        { label: "Available for Trials", value: data.availableForTrials === true ? "Yes" : data.availableForTrials === false ? "No" : "" },
        { label: "Available Immediately", value: data.availableImmediately === true ? "Yes" : data.availableImmediately === false ? "No" : "" },
        { label: "Can Travel", value: data.canTravel === true ? "Yes" : data.canTravel === false ? "No" : "" },
        { label: "Can Relocate", value: data.canRelocate === true ? "Yes" : data.canRelocate === false ? "No" : "" },
        { label: "Preferred Country", value: getLabel(COUNTRIES, data.preferredCountry) },
    ...
    },
    {
      ...
      fields: (() => {
        ...
        return [
          ...
          { key: "tiktok", label: "TikTok" }, { key: "youtube", label: "YouTube" },
          { key: "twitter", label: "X / Twitter" }, { key: "linkedin", label: "LinkedIn" },
        ].map(({ key, label }) => ({ label, value: links[key] || "" }));
      })(),
    },
  ];
  ...
  if (data.age > 0 && data.age < 18) {
    sections.splice(2, 0, {
      title: "Guardian Information", icon: User,
      fields: [
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold" style={{ color: "var(--fg-heading)" }}>Review Your Application</h2>
      ...
      <div className="rounded-xl p-4" style={{ backgroundColor: "var(--primary-lighter)", border: "1px solid var(--primary-light)" }}>
        <p className="text-sm" style={{ color: "var(--fg-text)" }}>
          By submitting this application, you confirm that all information provided is accurate and you agree to our terms of service and privacy policy.
        </p>
      </div>
    </div>
  );
